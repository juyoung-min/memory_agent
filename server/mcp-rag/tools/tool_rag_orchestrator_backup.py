...
async def get_model_client():
    """Create a new model client instance for each request"""
    client = InternalModelClient(config.model_mcp_url, timeout=config.model_timeout)
    return client

async def get_db_client():
    """Create a new db client instance for each request"""
    client = InternalDBClient(config.db_mcp_url, timeout=config.db_timeout)
    return client
...
async def rag_save_document(content: str, namespace: str = 'default', document_id: Optional[str] = None, metadata: Optional[Dict[str, Any]] = None, chunk: bool = True, collection: Optional[str] = None) -> str:
    ...
    # Create new clients for this request
...
Answer:"""
        ...
        # For EXAONE model, the response format is {"choices": [{"text": "..."}]}
        answer = ""
        if llm_response.get("choices") and len(llm_response["choices"]) > 0:
            answer = llm_response["choices"][0].get("text", "")
        else:
            # Fallback for other formats
            answer = llm_response.get("response", llm_response.get("message", ""))
